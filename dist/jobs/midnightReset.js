...
exports.runMidnightReset = runMidnightReset;
exports.startMidnightCron = startMidnightCron;
 ...
 */
function getMidnightOffsets(utcDate) {
 ...
 */
function getLocalYesterday(utcNow, utcOffsetMinutes) {
 ...
 */
function getLocalToday(utcNow, utcOffsetMinutes) {
    const localMsNow = utcNow.getTime() + utcOffsetMinutes * 60 * 1000;
 ...
 */
async function checkWeeklyForestCompletion(userId, dateStr) {
    const date = new Date(`${dateStr}T00:00:00.000Z`);
    // ISO week: Monday = 1, Sunday = 7
    // JS getUTCDay(): 0=Sun, 1=Mon, ..., 6=Sat → we only check on Sunday (end of ISO week)
    const dayOfWeek = date.getUTCDay(); // 0 = Sunday
    if (dayOfWeek !== 0)
        return; // Only check at the end of the ISO week (Sunday)
    ...
    const weekTrees = await prisma_1.prisma.dailyTree.findMany({
        where: {
            userId,
            date: {
                gte: mondayDate,
    ...
    if (weekTrees.length === 7) {
        ...
        console.log(`[midnightReset] 🌳 Weekly forest COMPLETE for user ${userId} — week ${weekLabel}`);
...
function getISOWeekLabel(monday) {
    ...
    const weekNum = Math.ceil(((monday.getTime() - jan4.getTime()) / 86400000 +
        jan4.getUTCDay() +
        1) /
        7);
...
async function runMidnightReset() {
    ...
    console.log(`[midnightReset] Running at ${utcNow.toISOString()} — targeting UTC offsets: ${targetOffsets.join(", ")} min`);
    ...
    try {
        await (0, leaderboardService_1.updateGroupsLeaderboard)();
 ...
 */
async function autoAbandonStaleSessions() {
    const now = new Date();
    ...
    const staleSessions = await prisma_1.prisma.session.findMany({
        where: {
            state: "active",
            expectedEndAt: {
                lte: cutoffTime,
    ...
    for (const session of staleSessions) {
        try {
            ...
            console.log(`[midnightReset] Auto-abandoned session ${session.id} for user ${session.userId} (expected end: ${session.expectedEndAt?.toISOString()}).`);
        }
        catch (err) {
...
async function processUserMidnight(userId, utcOffset, utcNow) {
    const yesterdayStr = getLocalYesterday(utcNow, utcOffset);
    const todayStr = getLocalToday(utcNow, utcOffset);
    const yesterdayDate = (0, streakService_1.toUtcMidnight)(yesterdayStr);
    const todayDate = (0, streakService_1.toUtcMidnight)(todayStr);
    // ------------------------------------------------------------------
    // Step 1: Finalise yesterday's tree
    // ------------------------------------------------------------------
    const existingTree = await prisma_1.prisma.dailyTree.findUnique({
    ...
    if (existingTree && existingTree.finalisedAt) {
        ...
        console.log(`[midnightReset] User ${userId}: yesterday (${yesterdayStr}) already finalised. Skipping.`);
        ...
        await prisma_1.prisma.dailyTree.update({
            where: { userId_date: { userId, date: yesterdayDate } },
            data: {
                isBare,
                finalisedAt: utcNow,
        ...
        console.log(`[midnightReset] User ${userId}: finalised ${yesterdayStr} — stage=${existingTree.stage}, sessions=${existingTree.totalSessions}, isBare=${isBare}`);
    }
    else {
        // No tree at all for yesterday — it was a missed day (bare soil)
        hadSessions = false;
        await prisma_1.prisma.dailyTree.create({
            data: {
                userId,
                date: yesterdayDate,
                stage: 0,
        ...
        console.log(`[midnightReset] User ${userId}: no tree for ${yesterdayStr} — created bare soil record.`);
    }
    // ------------------------------------------------------------------
    // Step 2: Update or reset streak
    // ------------------------------------------------------------------
    if (hadSessions) {
        await (0, streakService_1.upsertStreak)(userId, yesterdayDate);
        console.log(`[midnightReset] User ${userId}: streak incremented for ${yesterdayStr}.`);
    }
    else {
        await (0, streakService_1.resetStreak)(userId);
    ...
    // ------------------------------------------------------------------
    const nextDayExists = await prisma_1.prisma.dailyTree.findUnique({
    ...
    if (!nextDayExists) {
        ...
        console.log(`[midnightReset] User ${userId}: tree for ${todayStr} already exists (first session came in before cron). Skipping seed.`);
    }
    // ------------------------------------------------------------------
...
//# sourceMappingURL=midnightReset.js.map