...
exports.upsertStreak = upsertStreak;
 ...
 */
async function upsertStreak(userId, activeDate) {
    const existing = await prisma_1.prisma.streak.findUnique({ where: { userId } });
    if (!existing) {
        // First-ever session for this user — create streak row
        const created = await prisma_1.prisma.streak.create({
            data: {
                userId,
                currentStreak: 1,
                longestStreak: 1,
    ...
    const updated = await prisma_1.prisma.streak.update({
        where: { userId },
        data: {
