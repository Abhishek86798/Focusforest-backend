...
exports.upsertDailyTree = upsertDailyTree;
const prisma_1 = require("../lib/prisma");
// glow_level: 0–4
function computeGlowLevel(sessionsWithTask, totalSessions) {
    if (totalSessions === 0)
        return 0;
    return Math.min(4, Math.floor((sessionsWithTask / totalSessions) * 4));
}
// stage: 0–4. Thresholds: 1.0 / 2.0 / 3.0 / 4.0
function computeStage(cumulativeProgress) {
    if (cumulativeProgress >= 4.0)
        return 4;
    if (cumulativeProgress >= 3.0)
        return 3;
    if (cumulativeProgress >= 2.0)
        return 2;
...
async function upsertDailyTree(userId, stageProgressDelta, taskStatus) {
    ...
    const agg = await prisma_1.prisma.session.aggregate({
        where: {
            userId,
    ...
    const existing = await prisma_1.prisma.dailyTree.findUnique({
    ...
    const updated = await prisma_1.prisma.dailyTree.upsert({
        where: { userId_date: { userId, date: todayDate } },
        create: {
            userId,
            date: todayDate,
